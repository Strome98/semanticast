...
if (fs.existsSync(envPath)) {
  ...
  if (fs.existsSync(examplePath)) {
    console.warn('[config] .env file not found. You edited .env.example but must copy it to .env for values to load.');
  } else {
    console.warn('[config] No .env file present. Environment variables must be set externally.');
  }
}
