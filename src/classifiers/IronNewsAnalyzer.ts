...
function naiveBaseline(text: string): Classification {
  ...
  const bull = ["surge", "rally", "record", "higher", "growth", "upgrade"].some(
    (w) => t.includes(w)
  );
  const bear = ["plunge", "drop", "miss", "lower", "fraud", "downgrade"].some(
    (w) => t.includes(w)
  );
  if (bull && !bear)
    return { sentiment: "bullish", impact: "up", confidence: 0.7 };
  if (bear && !bull)
    return { sentiment: "bearish", impact: "down", confidence: 0.7 };
