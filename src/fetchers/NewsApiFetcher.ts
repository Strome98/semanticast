...
export class NewsApiFetcher {
  ...
  public async fetchEverything(
    query: string,
    {
      ...
      pageSize = 50,
      page = 1,
    }: { from?: string; pageSize?: number; page?: number } = {}
  ): Promise<Article[]> {
  ...
  public async fetchAllPages(
    query: string,
    {
      ...
      pageSize = 50,
      pageLimit = 3,
    }: { from?: string; pageSize?: number; pageLimit?: number } = {}
  ): Promise<Article[]> {
    const all: Article[] = [];
    ...
    all.forEach((article) => map.set(article.id, article));
    return [...map.values()];
  }
   ...
   */
  public buildRareEarthQuery(
    extraTerms: string[] = [],
    automotiveFocus: boolean = true
  ): string {
    const metalTerms = [
      '"rare earth"',
      "neodymium",
  ...
  private async fetchWithRetry(url: string, attempt = 1): Promise<Response> {
    ...
    if (res.status === 429) {
      if (attempt > 3)
        throw new Error("NewsAPI rate limit exceeded after retries");
...
function normalizeNewsApiArticle(article: any): Article {
  return {
    id: article.url, // use URL as id (unique enough)
    url: article.url,
